...
function obtenirStructureGraphique(livraisons, periode) {
  ...
  if (periode === 'aujourdhui') {
    ...
    fullDates = ['Semaine 1', 'Semaine 2', 'Semaine 3', 'Semaine 4'];
    const total30j = livraisons.filter(l => l.statut === 'livree').reduce((sum, l) => sum + (Number(l.montant_total) || 0), 0);
    dataPoints = [Math.round(total30j * 0.2), Math.round(total30j * 0.3), Math.round(total30j * 0.25), Math.round(total30j * 0.25)];
  } 
  else if (periode === 'annee') {
    labels = ['Jan', 'Fév', 'Mar', 'Avr', 'Mai', 'Juin', 'Juil', 'Août', 'Sep', 'Oct', 'Nov', 'Déc'];
    fullDates = ['Janvier', 'Février', 'Mars', 'Avril', 'Mai', 'Juin', 'Juillet', 'Août', 'Septembre', 'Octobre', 'Novembre', 'Décembre'];
    dataPoints = Array(12).fill(0);
    const moisActuel = maintenant.getMonth();
    const totalAnnee = livraisons.filter(l => l.statut === 'livree').reduce((sum, l) => sum + (Number(l.montant_total) || 0), 0);
    dataPoints[moisActuel] = totalAnnee;
  }
